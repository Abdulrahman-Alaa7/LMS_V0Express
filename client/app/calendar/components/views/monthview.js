...
// general utilities
import { getClosest, hextorgba, placePopup } from "../../utilities/helpers";
...
const monthWrapper =
  typeof window !== "undefined"
    ? document.querySelector(".monthview--calendar")
    : null;
...
export default function setMonthView(context, store, datepickerContext) {
  ...
  function createCellHeader(cell, day) {
    const daynumberAndMonth =
      day.getDate() + " " + locales.labels.monthsShort[day.getMonth()];
  ...
  function createCell(day, idx, entry, cellDate) {
    ...
    if (entry !== undefined && entry.length > 0) {
      ...
      entry = entry.filter(
        (el) => new Date(el.start).getMonth() === day.getMonth()
      );
  ...
  function getCoordinatesFromCell(cell) {
    return cell
      .getAttribute("data-mv-coordinates")
      .split(",")
      .map((x) => {
        return parseInt(x);
      });
  ...
  function configureForStorage(newCell, clone) {
    let [year, month, day] = getDateFromAttribute(
      newCell,
      "data-mv-date",
      "month"
    );
  ...
  function configGroupedCellOnDrop(groupedCell) {
    ...
    groupedCell.firstChild.firstChild.textContent = `${groupedCell.getAttribute(
      "data-mvgrouped-length"
    )} more...`;
  }
  ...
  function resetCellOnDrop(bxs, transition) {
    ...
    for (let i = 0; i < bxs.length; i++) {
      if (
        bxs[i] !== undefined &&
        bxs[i] !== null &&
        !bxs[i].classList.contains("monthview--box__form-temp")
      ) {
  ...
  function createGroupedCellOnDrop(cell, content, boxes, clone) {
    ...
    newGroupedCell.setAttribute(
      "data-mvgrouped-date",
      cell.getAttribute("data-mv-date")
    );
  ...
  function dragEngineMonth(e, box) {
    ...
    const mousemove = (e) => {
      ...
      document
        .querySelector(".current-drop-zone")
        ?.classList.remove("current-drop-zone");
      document
        .querySelector(`[data-mv-coordinates="${newX},${newY}"]`)
        .classList.add("current-drop-zone");
    };
  ...
  function createMoreModal(e) {
    ...
    modal.setAttribute(
      "style",
      `top: ${y}px; left: ${x}px; width: 216px; height: ${modalHeight}px; min-height: 120px; max-height: ${maxH}px;`
    );
    ...
    const parentDate = createDateFromFormattedString(
      e.target.parentElement.parentElement.getAttribute("data-mv-date")
    );
    ...
    dowspan.textContent =
      locales.labels.weekdaysShort[parentDate.getDay()].toUpperCase();
    ...
    resizeoverlay.addEventListener(
      "click",
      (e) => {
        closeMoreModal();
      },
  ...
  function dragOutOfModal(e) {
    ...
    const newLength =
      parseInt(moreModal.getAttribute("data-mvgrouped-length")) - 1;
  ...
  function handleMonthviewEditFormClose() {
    const tempcell = document.querySelector(
