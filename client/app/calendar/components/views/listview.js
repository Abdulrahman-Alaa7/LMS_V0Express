...
import { getClosest, hextorgba } from "../../utilities/helpers";
import {
  getdatearray,
  getFormDateObject,
...
const dateTimeTitle =
  typeof window !== "undefined"
    ? document.querySelector(".datetime-content--title")
    : null;
const listview =
  typeof window !== "undefined" ? document.querySelector(".listview") : null;
const listviewBody =
  typeof window !== "undefined"
...
export default function setListView(context, store, datepickerContext) {
  ...
  function createRowGroupCell(entry) {
    ...
    if (longerThanDay(start, end)) {
      ...
      datetitle = `${monthNames[end.getMonth()]} ${end.getDate()} ${
        tempyear ? tempyear : ""
      }`;
    } else {
