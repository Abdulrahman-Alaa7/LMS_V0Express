...
const datepickerBody =
  typeof window !== "undefined"
    ? document.querySelector(".datepicker__body--dates")
    : null;
const datepickerTitle =
  typeof window !== "undefined"
    ? document.querySelector(".datepicker-title")
    : null;
const datepickerChangeDate =
  typeof window !== "undefined"
    ? document.querySelector(".datepicker-change-date")
    : null;

// prev and next buttons aside from main app header datewrapper
const headerPrevBtn =
  typeof window !== "undefined" ? document.querySelector(".prev") : null;
const headerNextBtn =
  typeof window !== "undefined" ? document.querySelector(".next") : null;

const yearpickerTitle =
  typeof window !== "undefined"
    ? document.querySelector(".yearpicker-title")
    : null;
const monthpickerMonths =
  typeof window !== "undefined"
    ? document.querySelectorAll(".monthpicker__month")
    : null;
...
export default function setDatepicker(context, store, datepickerContext, type) {
  ...
  let count = 0;
  let hasweek;
  let testDateSelected =
    type === "form"
      ? datepickerContext.getDateSelected()
      : context.getDateSelected();
  ...
  function createCells(montharray) {
    ...
    for (let i = 0; i < montharray.length; i++) {
      ...
      if (
        compareDates(montharray[i], currentWeekStart) &&
        context.getComponent() === "week"
      ) {
        hasweek = true;
      }
      ...
      if (
        montharray[i].getDate() === testDateSelected &&
      ...
      if (groupedEntries.includes(formattedDate)) {
        if (
          !datename.classList.contains("datepicker__body--datename-today") &&
          !datename.classList.contains("datepicker__body--datename-selected")
        ) {
          datename.setAttribute("class", "datepicker__body--datename");
  ...
  function handleFormDate(y, m, d) {
    ...
    activeFormDate.textContent = `${datepickerContext
      .getMonthName()
      .slice(0, 3)} ${d}, ${y}`;

    const inactiveFormDate = document?.querySelector(".inactive-form-date");
    const inactiveValue = inactiveFormDate
    ...
    const inactiveDateType = inactiveFormDate.getAttribute(
      "data-form-date-type"
    );
     ...
     */
    if (
      (isBeforeDate(inactiveDate, datepickerDate) &&
        inactiveDateType === "end") ||
      (isBeforeDate(datepickerDate, inactiveDate) &&
        inactiveDateType === "start")
    ) {
      inactiveFormDate.setAttribute("data-form-date", `${y}-${m}-${d}`);
      inactiveFormDate.textContent = `${datepickerContext
        .getMonthName()
        .slice(0, 3)} ${d}, ${y}`;
    }
  }
  ...
  function setSelectedToPrevDay() {
    ...
    if (!prev || prev === undefined || prev === null) {
      ...
      last[last.length - 1].classList.add(
  ...
  function closeDatepicker() {
    ...
    if (type === "form") {
      document
        .querySelector(".active-form-date")
        ?.classList.remove("active-form-date");
    }
