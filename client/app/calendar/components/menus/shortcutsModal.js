const shortcutsModalOverlay =
  typeof window !== "undefined"
    ? document.querySelector(".shortcuts-modal-overlay")
    : null;
const shortcutsModal =
  typeof window !== "undefined"
    ? document.querySelector(".shortcuts__modal")
    : null;
const shortcutsModalContent =
  typeof window !== "undefined"
    ? document.querySelector(".shortcuts-modal-content")
    : null;
const shortcutsModalClose =
  typeof window !== "undefined"
    ? document.querySelector(".close-shortcuts-modal")
    : null;
const notifyShortcutsStatus =
  typeof window !== "undefined"
    ? document.querySelector(".keyboard-disabled-sm-two")
    : null;
...
export default function handleShortCutsModal(store) {
  ...
  function setStatusIcon(status) {
    if (status) {
      notifyShortcutsStatus.setAttribute(
        "data-tooltip",
        "Keyboard shortcuts enabled"
      );
      notifyShortcutsStatus.firstElementChild.setAttribute(
        "fill",
        "var(--primary1)"
      );
    } else {
  ...
  function handleShortcutsModalOpen() {
    ...
    for (let i = 0; i < Object.values(shortcuts).length; i++) {
      ...
      shortcutsModalContent.appendChild(
        createShortcut(value.shortcut, value.action)
      );
    }
