...
const ctgform =
  typeof window !== "undefined"
    ? document.querySelector(".category__form")
    : null;
const ctgformoverlay =
  typeof window !== "undefined"
    ? document.querySelector(".category__form-overlay")
    : null;
const ctgformInput =
  typeof window !== "undefined"
    ? document.querySelector(".category__form-input")
    : null;
const colorPickerTitle =
  typeof window !== "undefined"
    ? document.querySelector(".color-picker__title")
    : null;
const colorPickerOptions =
  typeof window !== "undefined"
    ? document.querySelector(".color-picker__options")
    : null;
const ctgErrMsg =
  typeof window !== "undefined"
    ? document.querySelector(".ctg-input--err")
    : null;
...
export default function createCategoryForm(
  ...
  function createPickerOptions(currentColor) {
    ...
    colors.forEach((color) => {
      for (let i = 1; i < 8; i++) {
        colorPickerOptions.append(createColorOption(color[i], currentColor));
      }
    });
  }
  ...
  function validateNewCategory(categoryName, color) {
    let trimName = categoryName
      .trim()
      .replace(/[^a-zA-Z0-9\s]+|\s{2,}/g, " ")
      .trim();
