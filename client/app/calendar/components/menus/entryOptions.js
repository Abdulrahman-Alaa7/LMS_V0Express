...
const entryOptionsOverlay =
  typeof window !== "undefined"
    ? document.querySelector(".entry__options--overlay")
    : null;
const entryOptionsWrapper =
  typeof window !== "undefined"
    ? document.querySelector(".entry__options")
    : null;
const entryOptionsHeaderWrapper =
  typeof window !== "undefined"
    ? document.querySelector(".entry__options--header")
    : null;
const entryOptionsDateHeader =
  typeof window !== "undefined"
    ? document.querySelector(".entry__options-date")
    : null;
const entryOptionsTimeHeader =
  typeof window !== "undefined"
    ? document.querySelector(".entry__options-time")
    : null;
const entryOptionTitle =
  typeof window !== "undefined" ? document.querySelector(".eob-title") : null;
const entryOptionDescription =
  typeof window !== "undefined"
    ? document.querySelector(".eob-description")
    : null;
const entryOptionCategoryIcon =
  typeof window !== "undefined"
    ? document.querySelector(".eob-category--icon")
    : null;
const entryOptionCategory =
  typeof window !== "undefined"
    ? document.querySelector(".eob-category")
    : null;

export default function getEntryOptionModal(
  context,
  store,
  entry,
  datepickerContext,
  ...
  function setEntryDefaults() {
    ...
    if (getDateTime.time !== null) {
      if (getDateTime.time === undefined) {
        ...
        if (daysSince === 0) {
          ...
          let minSince = Math.floor(
            (secondsDiff - hourSince * 1000 * 60 * 60) / (1000 * 60)
          );
          if (hourSince === 0) {
            if (minSince === 1) {
    ...
    entryOptionCategoryIcon.setAttribute(
      "fill",
      store.getCtgColor(entry.category)
    );
