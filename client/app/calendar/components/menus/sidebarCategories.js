...
const cWrapper =
  typeof window !== "undefined"
    ? document.querySelector(".sb__categories--body")
    : null;
...
const categoriesHeader =
  typeof window !== "undefined"
    ? document.querySelector(".sb__categories--header")
    : null;
const categoryHeaderCaret =
  typeof window !== "undefined" ? document.querySelector(".sbch-caret") : null;
// renders via menu click -- see ./renderViews.js

export default function handleSidebarCategories(
  context,
  store,
  datepickerContext
) {
  ...
  function createDeleteCategoryPopup(e) {
    ...
    if (!noEntries) {
      ...
      const handleWrapperClick = (e) => {
        if (
          e.target.id === "ctg-delete" ||
          e.target.id === "ctg-move" ||
          e.target.classList.contains("popup-delete-ctg__option--move") ||
          e.target.classList.contains("popup-delete-ctg__option--move-select")
        ) {
          return;
        } else {
          e.target
            .closest(".popup-delete-ctg__options")
            .querySelector("input").checked = true;
          document
            .querySelectorAll(".popup-delete-ctg__options")
            .forEach((el) => {
              // el.style.backgroundColor = "transparent";
              el.setAttribute(
                "style",
                `background-color: transparent; border: 2px solid ${ctgcolor};`
              );
          ...
          e.target
            ...
            .setAttribute(
              "style",
              `background-color: ${offsetColor}; border: 2px solid ${ctgcolor};`
            );

          e.target
            .closest(".popup-delete-ctg__options")
            .classList.add("popup-delete-act");
        }
      };
    ...
    const proceedDeletePopup = () => {
      if (!noEntries) {
        const selectedOption = document?.querySelector(
          "input[name='popup-delete-ctg__option']:checked"
        ).id;
        const selectedCategory = document.querySelector(
          ".popup-delete-ctg__option--move-select"
        ).value;
  ...
  function openCategoryOptionsMenu(e, data) {
    ...
    let [x, y] = placePopup(
      192,
      128,
      [e.clientX, parseInt(targetElement.getBoundingClientRect().top) - 8],
    ...
    function closeCategoryOptionsMenu(flag, chckbox) {
      ...
      if (flag) {
        chckbox.removeAttribute("style");
      }
      document.removeEventListener(
        "keydown",
        handleCloseOnEscapeCtgOptionsMenu
      );
    }
