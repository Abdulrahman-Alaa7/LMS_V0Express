...
function calcNewLeft(index: number) {
  let base = 0.1;
  let add = 0.05;
...
function setBoxWidthWeek(box: any, prepend: any, dataidx: any) {
...
function setBoxWidthDay(box: any, prepend: any, dataidx: any) {
  const attr = box.getAttribute(dataidx);
  switch (attr) {
    case `${prepend}one`:
      box.style.left = "calc((100% - 0px) * 0 + 0px)";
...
function handleOverlap(col: any, view: ViewType, boxes: any) {
  const collisions =
  ...
  for (let i = 0; i < collisions.length; i++) {
    const box: any = document.querySelector(
      `[${boxIdAttr}="${collisions[i].id}"]`
    );
    let idx = i;
    if (i >= 15) {
      idx -= 12;
...
function setStylingForEvent(clause: any, wrapper: any, store: any) {
  switch (clause) {
    case "dragstart":
      ...
      const resizeOverlayElement = document.querySelector(".resize-overlay");
      if (resizeOverlayElement) {
        resizeOverlayElement.classList.remove("hide-resize-overlay");
      }
    ...
    case "dragend":
      ...
      if (sidebarElement) {
        sidebarElement.classList.remove("sidebar--dragged-over");
      }

      const resizeOverlayElement2 = document.querySelector(".resize-overlay");
      if (resizeOverlayElement2) {
        resizeOverlayElement2.classList.add("hide-resize-overlay");
      }
...
function updateBoxCoordinates(box: any, view: ViewType, boxes: any) {
...
function setBoxTimeAttributes(box: any, view: ViewType) {
...
function createBox(col: any, entry: any, view: ViewType, color: any) {
  const baseClass = identifiers.boxClasses[view].base;
  const attrPrepend = identifiers.boxAttributes[view].prepend;
  const attrPrependTwo = identifiers.boxAttributes[view].prepentwo;
  const coord: any = entry.coordinates;
...
function getBoxDefaultStyle(y: any, backgroundColor: any) {
...
function resetStyleOnClick(view: ViewType, box: any) {
  box.setAttribute("class", identifiers.boxClasses[view].base);
  box.style.left = "calc((100% - 0px) * 0 + 0px)";
  box.style.width = "calc((100% - 4px) * 1)";
}

function createTempBoxHeader(view: ViewType) {
  const baseClass = identifiers.boxClasses[view].base;
  const boxheader = document.createElement("div");
...
function startEndDefault(y: any) {
...
function calcNewHourFromCoords(h: any, y: any) {
  return Math.floor((h + y) / 12.5 / 4);
}

function calcNewMinuteFromCoords(h: any, y: any) {
  return Math.floor(((h + y) / 12.5) % 4) * 15;
}

function calcDateOnClick(date: any, start: any, length: any) {
...
function getOriginalBoxObject(box: any) {
  return {
...
function resetOriginalBox(box: any, boxorig: any) {
