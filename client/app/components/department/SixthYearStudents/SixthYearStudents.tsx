...
import SignUpSixthYearStudent from "./SignUpSixthYearStudent";
...
const SixthYearStudent: FC<Props> = () => {
  ...
  const [edityear, setEditYaer] = useState(userData?.user?.yearSix.yearId);
  ...
  const matchingUsers = data?.users?.filter(
    (user: any) => user?.studentYearOfStudy === userData?.user?.yearSix?.yearId
  );
        ...
        <>
          {openSignUp && (
            <CustomModal
              ...
              setRoute={setRoute}
              component={SignUpSixthYearStudent}
              refetch={refetch}
            />
          )}
        </>
...
export default SixthYearStudent;
