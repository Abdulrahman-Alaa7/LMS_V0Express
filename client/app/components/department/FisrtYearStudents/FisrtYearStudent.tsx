...
import SignUpFisrtYearStudent from "./SignUpFirstYearStudent";
...
const FirstYearStudent: FC<Props> = () => {
  ...
  const [edityear, setEditYaer] = useState(userData?.user.yearOne.yearId);
  ...
  const matchingUsers = data?.users?.filter(
    (user: any) => user?.studentYearOfStudy === userData?.user?.yearOne?.yearId
  );
        ...
        <>
          {openSignUp && (
            <CustomModal
              ...
              setRoute={setRoute}
              component={SignUpFisrtYearStudent}
              refetch={refetch}
            />
          )}
        </>
...
export default FirstYearStudent;
