...
import SignUpThirdYearStudent from "./SignUpThirdYearStudent";
...
const ThirdYearStudent: FC<Props> = () => {
  ...
  const [edityear, setEditYaer] = useState(userData?.user?.yearThree.yearId);
  ...
  const matchingUsers = data?.users?.filter(
    (user: any) =>
      user?.studentYearOfStudy === userData?.user?.yearThree?.yearId
  );
  ...
  const handlePromote = async () => {
    ...
    const studentYearOfStudy = userData?.user?.yearFour?.yearId;
        ...
        <>
          {openSignUp && (
            <CustomModal
              ...
              setRoute={setRoute}
              component={SignUpThirdYearStudent}
              refetch={refetch}
            />
          )}
        </>
...
export default ThirdYearStudent;
