...
import SignUpFourthYearStudent from "./SignUpFourthYearStudent";
...
const FourthYearStudent: FC<Props> = () => {
  ...
  const [edityear, setEditYaer] = useState(userData?.user?.yearFour.yearId);
  ...
  const matchingUsers = data?.users?.filter(
    (user: any) => user?.studentYearOfStudy === userData?.user?.yearFour?.yearId
  );
  ...
  const handlePromote = async () => {
    ...
    const studentYearOfStudy = userData?.user?.yearFive?.yearId;
        ...
        <>
          {openSignUp && (
            <CustomModal
              ...
              setRoute={setRoute}
              component={SignUpFourthYearStudent}
              refetch={refetch}
            />
          )}
        </>
...
export default FourthYearStudent;
