...
import SignUpSecondYearStudent from "./SignUpSecondYearStudent";
...
const SecondYearStudent: FC<Props> = () => {
  ...
  const [edityear, setEditYaer] = useState(userData?.user?.yearTwo.yearId);
  ...
  const matchingUsers = data?.users?.filter(
    (user: any) => user?.studentYearOfStudy === userData?.user?.yearTwo?.yearId
  );
  ...
  const handlePromote = async () => {
    ...
    const studentYearOfStudy = userData?.user?.yearThree?.yearId;
        ...
        <>
          {openSignUp && (
            <CustomModal
              ...
              setRoute={setRoute}
              component={SignUpSecondYearStudent}
              refetch={refetch}
            />
          )}
        </>
