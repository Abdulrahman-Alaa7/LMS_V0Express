...
import { useChangePasswordProfessorMutation } from "../../../redux/features/users/userProfessorApi";
...
const ChangePasswordProfessor: FC<Props> = ({ user }) => {
  const [showOne, setShowOne] = useState(false);
  const [showTwo, setShowTwo] = useState(false);
  const [oldPassword, setOldPassword] = useState("");
  const [newPassword, setNewPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [changePasswordDepartment, { isSuccess, error }] =
...
export default ChangePasswordProfessor;
