...
const ProfileInfoProfessor: FC<Props> = ({ avatar, user }) => {
  const [name, setName] = useState(user && user.name);
  const [updateAvatarDepartment, { isSuccess, error }] =
    useUpdateAvatarProfessorMutation();
  const [editProfileDepartment, { isSuccess: forSuccess, error: forError }] =
    useEditProfileProfessorMutation();
      ...
      <div className={`w-full pl-6 800px:pl-10`}>
        <form onSubmit={handleSubmit}>
          <div className={`800px:w-[50%] m-auto block pb-4`}>
            <div
              className={`border p-2 mb-4 rounded-lg !w-[250px] flex justify-center items-center flex-col mx-auto dark:border-gray-600 `}
            >
              <FaMoneyCheckAlt size={40} className={`text-emerald-500	`} />
...
export default ProfileInfoProfessor;
