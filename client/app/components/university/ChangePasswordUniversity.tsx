...
import { useChangePasswordUniversityMutation } from "../../../redux/features/users/userUniversityApi";
...
const ChangePasswordUniversity: FC<Props> = ({ user }) => {
  const [showOne, setShowOne] = useState(false);
  const [showTwo, setShowTwo] = useState(false);
  const [oldPassword, setOldPassword] = useState("");
  const [newPassword, setNewPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [changePasswordUniversity, { isSuccess, error }] =
  ...
  const passwordChangeHandler = async (e: any) => {
    ...
    if (newPassword !== confirmPassword) {
      ...
      await changePasswordUniversity({
        oldPassword,
        newPassword,
      });
    }
...
export default ChangePasswordUniversity;
