...
import { useRegisterFacultyMutation } from "../../../../redux/features/auth/authApi";
import toast from "react-hot-toast";
import CustomModal from "../../../utils/CustomModal";
import { LiaUniversitySolid } from "react-icons/lia";
import VerificationFaculty from "./VerificationFaculty";
...
const SignUpFaculty: FC<Props> = ({}) => {
  const [show, setShow] = useState(false);
  const [register, { data, error, isSuccess }] = useRegisterFacultyMutation();
  ...
  const universityName = userData?.user?.name;
  useEffect(() => {
    if (isSuccess) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ name, email, password }) => {
      const data = { name, email, password, userCreatedById, universityName };
