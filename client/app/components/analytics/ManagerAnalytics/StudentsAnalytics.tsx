...
import { useGetStudnetsAnalyticsQuery } from "../../../../redux/features/analytics/analyticsApi";
import { useGetAllUsersStudentQuery } from "../../../../redux/features/users/userStudent";
import { styles } from "../../../styles/style";
import { HiOutlineUsers } from "react-icons/hi2";

type Props = {};

const StudentsAnalytics: FC<Props> = ({}) => {
  const { data: studentsUsers, isLoading: studentsUsersLoading } =
    useGetAllUsersStudentQuery({});
  const { data, isLoading } = useGetStudnetsAnalyticsQuery({});
                ...
                <div className="my-2 flex justify-between items-center gap-10">
                  <div className="flex flex-col justify-center items-center">
                    <p className=" font-semibold text-[30px]">
                      {studentsUsers?.users.length}
                    </p>
                    <p className={` text-[16px] text-[#666]`}>Total Students</p>
                  </div>
                </div>
              </div>
            </div>
