...
const SidebarUniversity: FC<Props> = ({
  user,
  openSidebar,
  setOpenSidebar,
}) => {
      ...
      <Box sx={{ display: "flex" }}>
        ...
        >
          ...
          <List>
            ...
            <Divider className="!border-[#9e9e9e29]" />
            <ListItem disablePadding>
              <Link href={`/faculty-area`} className="w-full">
                <ListItemButton
                  className={`hover:!bg-[#9e9e9e29] !rounded-lg flex items-center gap-4 !mx-2 !my-1  ${currentPagePath === "/faculty-area" && "!bg-[#9e9e9e29]"}`}
                >
                  <div>
                    <LiaUniversitySolid
                      size={25}
